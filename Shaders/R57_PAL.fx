...
float4 PS_PALR57(float4 pos : SV_Position, float2 coords : TEXCOORD0) : SV_Target
{
        ...
        // difference
        //float zoom = 8;
        //float4 sampled = monitor_sample(tex, coords, 1./zoom/output_size, IN);
        //float4 simple = float4(monitor(tex, coords, IN),1.);
        //return float4(length(sampled - simple)*5*sin(IN.frame_count/30.))+simple;
}
